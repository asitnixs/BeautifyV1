...
<style lang="scss">
  ...
  .choice {
    ...
    &.active {
      background: linear-gradient(90deg, rgba($accent-color, 0.55), rgba($accent-color-2, 0.55));
      color: $accent-color-3;
      box-shadow: 0 0 15px rgba($accent-color-2, 0.75);
      text-shadow: 0 0 15px rgba($accent-color-3, 0.75);
    }
  }
  ...
  .choices {
    ...
    border-image: linear-gradient(180deg, $accent-color, $accent-color-2) 1;
    color: $clickgui-text-color;
    padding: 7px 7px;
    display: flex;
    flex-wrap: wrap;
